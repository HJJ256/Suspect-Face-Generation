...
path_gan_sample_pkl = './asset_results/sgan2_ffhq_sample_pkl/'
...
for pathfile_pkl in list_pathfile_pkl:
    ...
    for i in range(num_cur):
        ...
        np.save(pathfile_cur+'_w.npy',w[i])
        i_counter += 1
    list_z.append(z)
    list_w.append(w)
...
w_concat = np.concatenate(list_w, axis=0)

pathfile_sample_z = os.path.join(path_gan_sample_img, filename_sample_z)
pathfile_sample_w = os.path.join(path_gan_sample_img, filename_sample_w)

with h5py.File(pathfile_sample_z, 'w') as f:
    f.create_dataset('z', data=z_concat)
with h5py.File(pathfile_sample_w, 'w') as f:
    f.create_dataset('w', data=w_concat)
